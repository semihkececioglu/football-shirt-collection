...
const EditShirt = () => {
  ...
  const { errors, validateField, validateForm, setFieldError, clearFieldError } = useFormValidation();
  ...
  useEffect(() => {
    if (data?.data) {
      ...
      setFormData({
        ...
        playerIssue: shirt.playerIssue || false,
        purchaseDate: shirt.purchaseDate ? new Date(shirt.purchaseDate) : new Date(),
        purchasePrice: shirt.purchasePrice || "",
        currentValue: shirt.currentValue || "",
        purchaseLocation: shirt.purchaseLocation || "",
        notes: shirt.notes || "",
      });
      // Initialize keepImages with all existing images
      setKeepImages(shirt.images || []);
    }
  }, [data]);
  ...
  const handleImageChange = (e) => {
    ...
    // Check total images (existing kept + new)
    if (keepImages.length + newImages.length + files.length > 5) {
  ...
  const reorderExistingImages = (oldIndex, newIndex) => {
    setKeepImages((prev) => arrayMove(prev, oldIndex, newIndex));
    setHasUnsavedChanges(true);
  };

  const reorderNewImages = (oldIndex, newIndex) => {
    setNewImages((prev) => arrayMove(prev, oldIndex, newIndex));
    setNewPreviews((prev) => arrayMove(prev, oldIndex, newIndex));
    setHasUnsavedChanges(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // Send keepImages array (images to keep from existing)
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-center">
            <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-slate-600 dark:border-slate-400 mx-auto"></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            {keepImages.length > 0 && (
              <div className="mb-3">
                ...
                <SortableImageGrid
                  images={keepImages}
                  previews={keepImages.map((img) => getImageThumbnail(img))}
            ...
            <div>
              ...
              <SortableImageGrid
                ...
                onAdd={keepImages.length + newImages.length < 5 ? handleImageChange : null}
                maxImages={5 - keepImages.length}
              />
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-3">
              <div>
                <Label htmlFor="teamName" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <Label htmlFor="season" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
                ...
                <div>
                  <Label htmlFor="type" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
              <div>
                <Label htmlFor="brand" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
              ...
              <div>
                <Label htmlFor="size" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
              ...
              <div>
                <Label htmlFor="condition" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
              ...
              <div>
                <Label htmlFor="color" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <Label htmlFor="playerName" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
                  Name
                </Label>
                <Input
                  id="playerName"
              ...
              <div>
                <Label htmlFor="playerNumber" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
                  Number
                </Label>
                <Input
                  id="playerNumber"
                  ...
                  placeholder="7"
                  className={`h-9 ${errors.playerNumber ? "border-red-500" : ""}`}
                />
                {errors.playerNumber && (
                  <p className="text-red-500 text-xs mt-1">{errors.playerNumber}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-2">
              <div>
                <Label htmlFor="competition" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
              ...
              <div className="flex flex-wrap gap-2">
                <div className="flex items-center space-x-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded border border-slate-200 dark:border-slate-700 flex-1 min-w-[120px]">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleChange("signed", checked)}
                  />
                  <label htmlFor="signed" className="text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                    Signed
                  </label>
                </div>
                ...
                <div className="flex items-center space-x-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded border border-slate-200 dark:border-slate-700 flex-1 min-w-[120px]">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleChange("matchWorn", checked)}
                  />
                  <label htmlFor="matchWorn" className="text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                    Match Worn
                  </label>
                </div>
                ...
                <div className="flex items-center space-x-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 rounded border border-slate-200 dark:border-slate-700 flex-1 min-w-[120px]">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => handleChange("playerIssue", checked)}
                  />
                  <label htmlFor="playerIssue" className="text-xs font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                    Player Issue
                  </label>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <Label htmlFor="purchasePrice" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
                    Purchase {getSymbol()}
                  </Label>
                  <Input
                    id="purchasePrice"
                ...
                <div>
                  <Label htmlFor="currentValue" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
                    Current {getSymbol()}
                  </Label>
                  <Input
                    id="currentValue"
              ...
              <div>
                <Label htmlFor="purchaseLocation" className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-lg p-4 border border-slate-200 dark:border-slate-800">
            ...
            <Textarea
              ...
              maxLength={500}
              className={`resize-none text-sm ${errors.notes ? "border-red-500" : ""}`}
            />
            {errors.notes && (
          ...
          <div className="flex gap-2 pt-1">
            ...
            <Button
              ...
              className="flex-1 h-9"
            >
              {updateShirt.isPending ? (
                <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" />
              ) : (
                <Save className="w-3.5 h-3.5 mr-1.5" />
              )}
              {updateShirt.isPending ? t("editShirt.updating") : t("editShirt.updateShirt")}
            </Button>
          </div>
        </form>
      </motion.main>
